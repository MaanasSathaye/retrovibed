...
func (t cmdMagnet) Run(ctx *cmdopts.Global) (err error) {
	for _, uri := range t.Magnets {
		...
		path := userx.DefaultDownloadDirectory(fmt.Sprintf("%s.torrent", m.ID.String()))
		...
		log.Println("NOOP MAGNET - Not implemented", userx.DefaultDownloadDirectory(fmt.Sprintf("%s.torrent", m.ID.String())), spew.Sdump(m))
	}
