...
func MetadataInsertWithDefaults(
	...
	gql.Into("torrents_metadata").Default("created_at", "updated_at", "hidden_at", "initiated_at", "paused_at", "downloaded", "next_announce_at", "verify_at").Conflict("ON CONFLICT (id) DO UPDATE SET updated_at = DEFAULT, tracker = EXCLUDED.tracker")
}
...
func MetadataCompleteByID(
	...
	gql = gql.Query(`UPDATE torrents_metadata SET updated_at = NOW(), verify_at = NOW(), downloaded = {downloaded}, uploaded = {uploaded}, peers = {peers}, seeding = (bytes == {downloaded}) WHERE "id" = {id} RETURNING ` + MetadataScannerStaticColumns)
}
